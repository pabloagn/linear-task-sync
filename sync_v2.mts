...
function inferCoreSystemsLabel(projectIdentifier: string): string {
  const systemNumber = parseInt(projectIdentifier.split('.')[0].slice(1, 4));
  const lowerBound = Math.floor(systemNumber / 100) * 100;
  const upperBound = lowerBound + 99;
  return `[${lowerBound.toString().padStart(3, '0')}-${upperBound.toString().padStart(3, '0')}] System`;
}

function inferCoreAreasLabel(projectIdentifier: string): string {
  const areaNumber = projectIdentifier.split('.')[0].slice(1, 4);
  return `[${areaNumber}] Architecture`;
}
...
async function syncTasks() {
  try {
    ...
    const issuesToUpdate = issues.filter(issue => {
      ...
      const coreSystemsLabel = inferCoreSystemsLabel(issue.project.name);
      console.log(coreSystemsLabel);
      const coreAreasLabel = inferCoreAreasLabel(issue.project.name);
    ...
    for (const issue of issuesToUpdate) {
      ...
      if (coreSystemsLabelId) {
        ...
        console.warn(`Required label "${coreSystemsLabel}" not found in workspace labels`);
      }

      if (coreAreasLabelId) {
        newLabelIds.add(coreAreasLabelId);
      } else {
        console.warn(`Required label "${coreAreasLabel}" not found in workspace labels`);
      }
